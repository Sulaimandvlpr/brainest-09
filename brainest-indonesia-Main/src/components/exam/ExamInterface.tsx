...
export default function ExamInterface() {
  ...
  // Forum diskusi per soal (dummy, per question.id)
  const [comments, setComments] = useState<{[key:string]: {id:number, name:string, text:string, likes:number}[]}>({});
  const [newComment, setNewComment] = useState('');
  const [likeState, setLikeState] = useState<{[id:number]: boolean}>({});
  const commentInputRef = useRef<HTMLInputElement>(null);

  const currentComments = comments[examData?.questions[currentQuestion].id] || [
    { id: 1, name: 'Rizky', text: 'Jawaban B atau C ya? Ada yang bisa jelaskan?', likes: 1 },
    { id: 2, name: 'Sulaiman', text: 'Aku pilih B, karena ...', likes: 2 },
  ];
  ...
  const handleAddComment = (e: React.FormEvent) => {
    ...
    if (newComment.trim()) {
      setComments({
        ...comments,
        [examData?.questions[currentQuestion].id]: [
          ...(comments[examData?.questions[currentQuestion].id] || currentComments),
          { id: Date.now(), name: 'Kamu', text: newComment, likes: 0 },
        ],
      });
      setNewComment('');
  ...
  const handleLike = (id: number) => {
    setComments({
      ...comments,
      [examData?.questions[currentQuestion].id]: (comments[examData?.questions[currentQuestion].id] || currentComments).map(c => c.id === id ? { ...c, likes: c.likes + (likeState[id] ? -1 : 1) } : c)
    });
    setLikeState({ ...likeState, [id]: !likeState[id] });
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 h-full">
        ...
        <div className="lg:col-span-3 p-6 overflow-y-auto">
          ...
          {/* Forum Diskusi Soal */}
          <div className="mt-8 rounded-2xl bg-blue-3d-light/40 border border-cyan/20 shadow-inner p-4">
            <div className="flex items-center gap-2 mb-3">
              <MessageCircle className="w-5 h-5 text-cyan-400" />
              <span className="font-bold text-white text-lg">Diskusi Soal Ini</span>
            </div>
            <div className="space-y-3 max-h-40 overflow-y-auto custom-scrollbar pr-2">
              {currentComments.length === 0 && <div className="text-blue-200 text-sm">Belum ada diskusi. Jadilah yang pertama!</div>}
              {currentComments.map((c) => (
                <div key={c.id} className="flex items-start gap-3 bg-blue-3d/40 rounded-xl p-3">
                  <div className="flex-shrink-0 w-9 h-9 rounded-full bg-cyan-700 flex items-center justify-center text-white font-bold text-lg shadow-3d">{c.name[0]}</div>
                  <div className="flex-1">
                    <div className="font-semibold text-cyan-200">{c.name}</div>
                    <div className="text-white text-sm mb-1">{c.text}</div>
                    <button onClick={() => handleLike(c.id)} className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full transition ${likeState[c.id] ? 'bg-cyan-500 text-white' : 'bg-cyan-900/30 text-cyan-200 hover:bg-cyan-700/40'}`}>
            ...
            <form onSubmit={handleAddComment} className="flex gap-2 mt-4">
              <input
                ref={commentInputRef}
                type="text"
                className="flex-1 rounded-full px-4 py-2 bg-blue-3d/60 border border-cyan/30 text-white placeholder:text-blue-200 focus:outline-none focus:ring-2 focus:ring-cyan-400 shadow"
                placeholder="Tulis komentar atau pertanyaan..."
                value={newComment}
                onChange={e => setNewComment(e.target.value)}
              />
              <Button type="submit" variant="3d" className="rounded-full px-6">Kirim</Button>
            </form>
          </div>
        </div>
      </div>
